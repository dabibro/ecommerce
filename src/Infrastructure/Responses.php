...
class Responses
{
    public static function getResponse($varRespCode)
    {
        ...
        $returnArray = array('success' => $success, 'response' => $response, 'message' => $responseDescription);
        return $returnArray;
    ...
    public static function alertResponse($message = "", $alert = "")
    {
        ...
        switch ($alert):
            case "danger" :
                $response = "<div class=\"alert alert-danger\"><i class=\"ri-error-warning-line icon mr-2\"></i> " . $message . " <button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button></div>";
                break;
            case "success" :
                $response = "<div class=\"alert alert-success\"><i class=\"fal fa-check-circle\"></i> " . $message . " <button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button></div>";
                break;
            case "warning" :
                $response = "<div class=\"alert alert-warning\"><i class=\"fal fa-exclamation-circle\"></i> " . $message . " </div>";
                break;
            case "info" :
                $response = "<div class=\"alert alert-info\"><i class=\"fal fa-info-circle\"></i> " . $message . " <button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button></div>";
                break;
            default:
                $response = "<div class=\"alert alert-primary\"><i class=\"fal fa-question-circle\"></i> " . $message . " <button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button></div>";
                break;
        endswitch;
        return $response;
    }

    public static function alertResponseUI($message, $type): string
    {
        return ('<div class="alert alert-right alert-' . $type . ' alert-dismissible fade show " role="alert">
            <span>' . $message . ' . </span>
